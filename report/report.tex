...
Gaussian process regression \cite{rasmussen2006gaussian}

\section{Results}
\subsection{Principal Component Regression}
As for PCR, we only consider the result on the data after normalization. 

We can see the result predicted by PCR in Figure \ref{pic4}. And the SMSE is 0.100806.
\begin{figure}[h]
  \centering
  % Requires \usepackage{graphicx}
...
Figure \ref{pic8} shows the change of SMSE as the parameter $\alpha$ of lasso regression is increasing. As we can see in the figure, both the SMSE of PCR and PLSR are increasing. That means there are no over-fitting in the two regression methods before, and smoothing the curve of $\bfv$ is not a good way to improve the performance of those methods.
